...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    # ------------------------------------------------------------------ outbound
    async def _is_handoff_active(self, chat_id: str) -> bool:
        """Checa se um operador assumiu manualmente essa conversa pelo
        painel (tenant-panel), caso em que o bot deve ficar em silêncio.

        Consulta um endpoint HTTP local exposto pelo sidecar mongo-sync
        (mesmo pod, único processo com acesso ao Mongo compartilhado — o
        container hermes não fala com o Mongo diretamente). Falha aberta:
        qualquer erro/timeout aqui NUNCA deve impedir o bot de responder
        normalmente, então trata como "sem handoff" nesse caso.
        """
        if self._http_client is None:
            return False
        try:
            resp = await self._http_client.get(
                "http://127.0.0.1:8091/handoff",
                params={"chat_id": chat_id},
                timeout=2.0,
            )
            if resp.status_code != 200:
                return False
            return bool(resp.json().get("handoff"))
        except Exception:
            return False
    ...
    async def send(
        ...
        if await self._is_handoff_active(chat_id):
            logger.info(
                "[whatsapp_cloud] send suppressed for chat_id=%s (handoff manual ativo)",
                chat_id,
            )
            return SendResult(success=True, message_id=None)
